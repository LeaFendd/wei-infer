...
TEST(test_tensor, to_cpu) {
    ...
    auto alloc_cu = CUDADeviceAllocatorFactory::getInstance();
...
TEST(test_tensor, clone_cuda) {
    ...
    cudaMemcpy(
        p2, t2_cu.ptr<float>(), sizeof(float) * 32 * 32, cudaMemcpyDeviceToHost
    );
    for (int i = 0; i < 32 * 32; ++i) {
        ASSERT_EQ(p2[i], 1.f);
    }
    ...
    cudaMemcpy(
        p2, t1_cu.ptr<float>(), sizeof(float) * 32 * 32, cudaMemcpyDeviceToHost
    );
    for (int i = 0; i < 32 * 32; ++i) {
        ASSERT_EQ(p2[i], 1.f);
    }
...
TEST(test_tensor, clone_cpu) {
    using namespace base;
    auto alloc_cpu = CPUDeviceAllocatorFactory::getInstance();
