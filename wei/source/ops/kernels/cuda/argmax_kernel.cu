...
__forceinline__ __device__ void block_reduce_argmax(
    float &val, size_t &ptr, float *shared_value, size_t *shared_ptr
) {
...
__global__ void
argmax_kernel_fp32(const float *input_ptr, size_t size, size_t *output_idx) {
...
size_t argmax_kernel_cu(const float *input_ptr, size_t size, void *stream) {
    ...
    if (!stream) {
        ...
        cudaMemcpy(
            &output_index, index, sizeof(size_t), cudaMemcpyDeviceToHost
        );
    } else {
