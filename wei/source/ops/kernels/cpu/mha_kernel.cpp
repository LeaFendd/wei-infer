...
void mha_kernel(
    ...
    for (int32_t h = 0; h < head_num; ++h) {
        ...
        for (int32_t t = 0; t <= pos; t++) {
            ...
            get_matmul_kernel(device_type)(
                query_mat, key_mat, score_mat, scale, config
            );
        }

        tensor::Tensor score_head_tensor(
            base::DataType::kDataTypeFp32,
            pos + 1,
            false,
            nullptr,
        ...
        get_softmax_kernel(device_type)(
            score_head_tensor, config ? config->stream : nullptr
        );
        ...
        get_scale_sum_kernel(device_type)(
            value_tensor,
            score_head_tensor,
            output_tensor,
            pos,
