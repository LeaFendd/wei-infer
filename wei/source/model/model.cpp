...
Model::Model(
    ...
    : model_type_(model_type), token_path_(std::move(token_path)),
...
base::Status
Model::insert_buffer(ModelBufferType buffer_idx, const tensor::Tensor &tensor) {
...
base::Status Model::generate_model_infos(const ModelConfig &config) const {
    ...
    if (std::abs(config.vocab_size) != config_->vocab_size_) {
        return base::error::ModelParseError("Vocabulary size mismatch between "
                                            "the model file and the token list."
        );
    }
    return base::error::Success();
}
...
base::Status Model::create_encode_layer() {
    ...
    encode_layer_ = std::make_unique<op::EncodeLayer>(
        device_type_, true, false, std::move(spe)
    );
    if (!encode_layer_) {
