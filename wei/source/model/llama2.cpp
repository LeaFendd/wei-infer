...
#include "../ops/kernels/cpu/rope_kernel.h"
#include "../ops/kernels/cuda/rope_kernel.cuh"
...
base::Status LLama2Model::forward(
...
void LLama2Model::create_param_layers() {
    ...
    for (int32_t i = 0; i < config_->layer_num_; ++i) {
        auto wk = std::make_shared<op::MatmulLayer>(
            device_type_, config_->kv_dim_, dim
        );
        wk->set_weight(
            0,
            {config_->kv_dim_, dim},
...
void LLama2Model::init_mem() {
    ...
    if (device_type_ == base::DeviceType::kDeviceCPU) {
        alloc = base::CPUDeviceAllocatorFactory::getInstance();
    } else {
        alloc = base::CUDADeviceAllocatorFactory::getInstance();
    }
    ...
    std::shared_ptr<base::DeviceAllocator> alloc_cpu =
        base::CPUDeviceAllocatorFactory::getInstance();
    std::shared_ptr<base::DeviceAllocator> alloc_cu =
        base::CUDADeviceAllocatorFactory::getInstance();

    tensor::Tensor input_tokens(
        base::DataType::kDataTypeInt32, 1, true, alloc_cpu
    );
    tensor::Tensor input_embeddings(
        base::DataType::kDataTypeFp32, 1, config_->dim_, true, alloc
    );
    ...
    tensor::Tensor rms_output(
        base::DataType::kDataTypeFp32, config_->dim_, true, alloc
    );
    ...
    tensor::Tensor w1_output(
        base::DataType::kDataTypeFp32, config_->hidden_dim_, true, alloc
    );
    tensor::Tensor w3_output(
        base::DataType::kDataTypeFp32, config_->hidden_dim_, true, alloc
    );
    ...
    tensor::Tensor forward_output(
        base::DataType::kDataTypeFp32, config_->vocab_size_, true, alloc
    );
    if (device_type_ == base::DeviceType::kDeviceCUDA) {
        tensor::Tensor forward_output_cpu(
            base::DataType::kDataTypeFp32, config_->vocab_size_, true, alloc_cpu
        );
        CHECK(insert_buffer(
            ModelBufferType::kForwardOutputCPU, forward_output_cpu
        ));
    }
...
base::Status LLama2Model::create_layers() {
    ...
    if (llama_layers_->wq_layers_.size() != config_->layer_num_ ||
        ...
        return error::InternalError("Create the matmul layer in the attention "
                                    "and ffn attention layers for "
                                    "the llama model "
                                    "failed.");
    }
    ...
    for (int32_t i = 0; i < config_->layer_num_; ++i) {
        if (!llama_layers_->wq_layers_.at(i) ||
            !llama_layers_->wk_layers_.at(i) ||
            !llama_layers_->wv_layers_.at(i) ||
            !llama_layers_->wo_layers_.at(i)) {
            return error::InternalError(
    ...
    if (llama_layers_->w1_layers_.size() != config_->layer_num_ ||
        ...
        return error::InternalError("Create the matmul layer in the "
                                    "feedforward layers for the llama model "
                                    "failed.");
    }
...
op::EmbeddingOutput LLama2Model::embedding(const std::vector<int> &tokens
    ...
    auto input_token_num = tensor::Tensor(
        base::DataType::kDataTypeInt32, static_cast<int32_t>(tokens.size())
    );
...
void LLama2Model::attention_qkv(
    int32_t layer_idx, const tensor::Tensor &pos_tensor
) const {
    CHECK(llama_layers_ != nullptr);
    // kv cache
...
base::Status LLama2Model::predict(
...
void LLama2Model::attention_mha(
    int32_t layer_idx, const tensor::Tensor &pos_tensor
) const {
    CHECK(llama_layers_ != nullptr);
    // mha
    ...
    STATUS_CHECK(mha_layer->forward(
        query, score_storage, key_cache, val_cache, mha_output
    ));
...
void LLama2Model::cls_logits(const tensor::Tensor &input) const {
    CHECK(llama_layers_ != nullptr);
    const auto &norm =
        llama_layers_->rmsnorm_layers_.at(2 * config_->layer_num_);
...
int32_t
LLama2Model::post_processing(const tensor::Tensor &pos, bool is_prompt) const {
    tensor::Tensor forward_output = get_buffer(ModelBufferType::kForwardOutput);
